...
\begin{document}
    \begin{defn}[Causal Space \cite{axioms-1}]
    ...
    \begin{proof}
        ...
        \end{align*}
        where ${\diamondsuit}$ follows from applying the rephrased axiom again, but this time to ${K_S(\omega_S, B)}$. 
    \end{proof}
    \sid{See the alternate notation and proof (the one I originally had) commented out below for clarity.}
    \begin{figure}
        \centering
...
%     *********************************************
    ...
    Let ${\mathfrak{C} = (\prod_{t\in T}\Omega_t, \otimes_{t\in T}\mathcal{E}_t, \mathbb{P}, \mathbb{K})}$ be a causal space and ${U, V\in 2^T}$ be disjoint subsets of variables. Then,
    \begin{align*}
    ...
    $${K_{U\to V}(\omega_U, A) = K_U(\omega_X, A)\foralll A \in \mathcal{H}_V}$$ (compatibility with the original causal mechanism).
    ...
    is a Markov Kernel that satisfies 
         $${K_{X\to Y}(\omega_X, A) = K_X(\omega_X, A)\foralll A \in \mathcal{H}_Y}$$ (compatibility with the original causal mechanism).
    ...
    \end{ex}
    \begin{rem}[Kernels between from a subset to a superset of variables]
        \label{rem:Kernels between subsets of variables}
            Observe that \autoref{lem:Rephrasing-axiom-2} also enables us to define kernels ${K_{U\to V}:\Omega_U\times H_V\to [0,1]}$ for all ${U\subset V}$ by
            \begin{align*}
                K_{U\to V}(\omega_U, A) &= K_U(\omega_U, A\cap\pi_U^{-1}(\omega_U))\\
                &= \delta_{\omega_U}(A_U)K_U(\omega_U, \pi^{-1}_{U\cup \{V\setminus U\}}(\omega_U\times A_{V\setminus U}))
            \end{align*}
            for all ${\omega_U\in \Omega_U}$ and ${A\in \mathcal{H}_V}$. The ${\delta{\omega_U}(\nu_U)}$ term ensures that we get ${0}$ if ${A\cap \pi^{-1}_U(\omega_U)=\emptyset}$ which can only occur if ${\omega_U\notin A\in \mathcal{H}_V}$ since ${V\supset U}$ In the singleton case we get 
            \begin{align*}
                K_{U\to V}(\omega_U, \nu_V) = \delta_{\omega_U}(\nu_U)K_U(\omega_U, \begin{pmatrix}
                    \omega_U \\ \nu_{V\setminus U}
                \end{pmatrix}). 
            \end{align*}
        \end{rem}
    \begin{rem}[Causal mechanisms encode interventional distributions]
    \label{rem:Causal-mechanisms-encode-interventional-distributions}
        One of the key features of causal spaces is that interventional distributions are explicitly specified by the causal kernels. To see this consider a kernel ${K_{X\to Y}}$ between distrinct variables as defined in \autoref{cor:Kernels-between-variables}. Let ${x_0\in \Omega_X}$ be arbitrary and ${A\in \mathcal{H}_Y}$ be arbitrary, then 
        \begin{align*}
            K_{X\to Y}(x_0, A) &= K_X(x_0, \pi_{X,Y}^{-1}(x_0\times A_Y))\\
                               &= K_X(x_0, A)\\
                               &\stackrel{\spadesuit}{=} \int_{\Omega_X}\delta_{x_0}(d\omega_X)K_X(\omega_X,B). 
        \end{align*}
        Intuitively, ${K_{X\to Y}(x_0, y_0)}$ computes ${\mathbb{P}^{\mathfrak{C};\text{do}(X=x_0)}(Y\in A_y)}$. The integral computes the interventional distribution by first placing the distribution ${\delta_{x_0}}$ on ${\mathcal{H}_X}$ and then propogating its effect via the causal mechanism ${K_X}$. The expansion in terms of the integral enables us to replace the Dirac measure in ($\spadesuit$) with an arbitrary probability measure ${\mathbb{Q}}$ on ${\mathcal{H}_Y}$ would compute the probability of ${A_y}$ after intervening on ${\mathcal{H}_X}$ by ${\mathbb{Q}}$.
    \end{rem}
    Each causal kernel ${K_U(\omega_U, A)}$ computes an interventional ${\mathbb{P}^{\mathfrak{C};\text{do}(X_u = \omega_U)}(A)}$, but often we want to construct a new causal space reflecting the intervention itself. This new space has observational distribution equal to the interventional distribution (for instance the observational distribution on the intervened-upon sub-$\sigma$-algebra $\mathcal{H}_U$ matches the intervention), and causal mechanisms updated to reflect soft interventions. This is analogous to obtaining a new graphical model by replacing edited arrows and distributions after intervention.

    \begin{defn}[Interventions \cite{axioms-1}]
        \label{defn:Interventions}
            Let ${\mathfrak{C} = (\prod_{t\in T}\Omega_t, \otimes_{t\in T}\mathcal{E}_t, \mathbb{P}, \mathbb{K})}$ be a causal space, ${U\in 2^T}$, ${\mathbb{Q}}$ be a probability measure on ${\mathcal{H}_U}$, and ${\mathbb{L}=\{L_V : V\in 2^U\}}$ be a causal mechanism on ${(\Omega_V,\mathcal{H}_U, \mathbb{Q})}$. An intervention on ${\mathcal{H}_U}$ via ${(\mathbb{Q}, \mathbb{L})}$ is a new causal space \\${(\prod_{t\in T}\Omega_t, \otimes_{t\in T}\mathcal{E}_t, \mathbb{P}^{\text{do}(U, \mathbb{Q})}, \mathbb{K}^{\text{do}(U, \mathbb{Q}, \mathbb{L})})}$, where 
            \begin{align*}
                \mathbb{P}^{\text{do}(U, \mathbb{Q})}(A) &= \int_{\Omega_U} \mathbb{Q}(d\omega_U)K_U(\omega_U,A)
            \end{align*}
            and
            \begin{align*}
                K_S^{\text{do}(U, \mathbb{Q}, \mathbb{L})}(\omega_S, A) &= \int_{\omega_U} L_{S\cap U}(\omega_{S\cap U},d\nu_U)K_{S\cup U}((\omega_{S\setminus U}, \nu_U), A)
            \end{align*}
            for all ${A\in \mathcal{H}}$ and ${\omega_S\in \Omega_S}$.
    \end{defn}

    Axiom 2 enforces an important condition in causal inference, namely that intervening on a sub-${\sigma}$-algebra ${\mathcal{H}_U}$ by placing a probability distribution ${\mathbb{Q}}$ results in the observational distribution of ${\mathcal{H}_U}$ being precisely ${\mathbb{Q}}$ after intervention. Intuitively, in the graphical model framework, intervening on a variable ${X}$ by placing a Dirac measure ${\delta_a}$ on it results in the interventional distribution of ${X}$ being precisely this point measure. One desirable property that the axioms of causal spaces do not enforce is \emph{consistency}, i.e.  . The following is a counter-example. 
    \begin{ex}[Causal space that fails compositionality]
    \label{ex:Causal-space-that-fails-compositionality}
        Consider the following causal space on ${3}$ variables with countable sample  spaces  (i.e. ${T=\{X,Y<Z\}}$, and ${\Omega = \Omega_X\times\Omega_Y\times\Omega_Z = \mathbb{N}^3}$). Since the sample spaces are countable, we can define the kernels on singleton events and extend them to the entire ${\sigma}$-algebra by summing. We focus on the causal mechanisms involving interventions on ${X}$.  Let ${\mathbb{P}_N}$ be a jointly independent distribution over noise variables ${(N_X, N_Y, N_Z)}$. 
        \begin{align*}
            K_X(\omega_X, (\nu_X,\nu_Y,\nu_Z )) &= \delta_{\omega_X}(\nu_X)\mathbb{P}_N(\{(n_X, n_Y,n_Z): \nu_Y = 2\nu_X+n_Y, \nu_Z = \nu_X+\nu_Y+n_Z \})\\
            &= \delta_{\omega_X}(\nu_X)\mathbb{P}_N(\{(n_Y,n_Z): \nu_Y = 2\omega_X+n_y, \nu_Z = 3\omega_X+n_Y+n_Z \})\\
            K_{X,Y}(\begin{pmatrix}
                \omega_X\\ \omega_Y
            \end{pmatrix}, \begin{pmatrix}
                \nu_X\\ \nu_Y \\ \nu_Z
            \end{pmatrix}) &= \delta_{\begin{pmatrix}
                \omega_X\\ \omega_Y
            \end{pmatrix}}\begin{pmatrix}
                \nu_X\\ \nu_Y
            \end{pmatrix}\mathbb{P}_N(\{(n_X, n_Y,n_Z): \nu_Z = 2\nu_X+\nu_y+n_Z \})\\
            &= \delta_{\begin{pmatrix}
                \omega_X\\ \omega_Y
            \end{pmatrix}}\begin{pmatrix}
                \nu_X\\ \nu_Y
            \end{pmatrix}\mathbb{P}_N(\{n_Z: \nu_Z = 2\omega_X+\omega_y+n_Z \})
        \end{align*}
        By \autoref{cor:Kernels-between-variables}, 
        \begin{align*}
            K_{X\to Y}(\omega_X, \nu_Y) &= K_X(\omega_X, \omega_X\times\nu_Y\times \Omega_Z)\\
            &= \sum_{\mu_Z\in \Omega_Z}\delta_{\omega_X}(\omega_X)\mathbb{P}_N(\{(n_Y,n_Z): \nu_Y = 2\nu_X+n_Y, \mu_Z = 3\omega_X+n_Z \}) \\
            &= \sum_{\mu_Z\in \Omega_Z}\mathbb{P}_N(\{(n_X, n_Y,n_Z): \nu_Y = 2\omega_X+n_y, \mu_Z = 3\omega_X+n_Y+n_Z \}) \\
            &= \mathbb{P}_N(\{(n_X, n_Y,n_Z): \nu_Y = 2\omega_X+n_y \})
        \end{align*}  		
    \end{ex}
    
    
    We want to discuss when a collection of variables has a causal effect on a given event ${A}$.  
    \begin{defn}[Effects in causal spaces \cite{axioms-1}]
    ...
    \end{defn}
    \begin{nota}
        We are often interested in causal effects between two variables. We say that ${X}$ is non-/actively/dormantly causal to ${Y}$ if ${\mathcal{H}_X}$ is non-/actively/dormantly to ${\mathcal{H}_Y}$. We say that ${X}$ is causal to ${Y}$ if ${X}$ is either actively or dormantly causal to ${Y}$.
    \end{nota}
    Analogously to \autoref{defn:SCM-Ancestors}, we can define ancestors in the causal space framework. 
    \begin{defn}[Ancestors]
    \label{defn:Ancestors}
        Let ${\mathfrak{C}=(\prod_{t\in T} E_t, \otimes_{t\in T} \mathcal{E}_t, \mathbb{P}, \mathbb{K})}$ be a causal space and ${X\in T}$ be an arbitrary variable. The set of ancestors of ${X}$ is
        \begin{align*}
            \text{An}_{\mathfrak{C}}(X) = \{Y\in T\setminus X: Y \text{ is causal to } X\}.
        \end{align*}
    \end{defn}
